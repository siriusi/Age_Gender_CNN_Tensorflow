...
def age_gender_model(inputs, is_training, outputs_age = None, outputs_gender = None, num_class = 80, batch_size = 100, learning_rate = 1e-5):
    ...
    cnn_net_gender = softmax(cnn_net_gender, name='probs_gender')
    ...
    return cnn_train, cnn_net_age, cnn_net_gender, total_loss, age_loss, gender_loss, \
            cnn_MAE_age, cnn_accuracy_gender, cnn_predictions_age, cnn_predictions_gender
...
def run_test(session, input_data, model_path = "train_models/age_gender_tensornets_wiki_cropface_model_20180519_123211"):
    ...
    loader.restore(session, tf.train.latest_checkpoint("train_models/"))
    inference_graph = tf.get_default_graph()

    cnn_predictions_age = session.graph.get_tensor_by_name("mobilenet100/cnn_predictions_age:0")
    cnn_predictions_gender = session.graph.get_tensor_by_name('mobilenet100/cnn_predictions_gender:0')
    
    inputs_tensor = session.graph.get_tensor_by_name('inputs_tensor:0')
    is_training = session.graph.get_tensor_by_name('is_training:0')
    
    input_data = nets.preprocess('mobilenet100', input_data)
    
    feed = {inputs_tensor : input_data, is_training : False} 
    cnn_predictions_age_test, cnn_predictions_gender_test = session.run([cnn_predictions_age, cnn_predictions_gender], feed_dict=feed)
...
def run_test_temp(sess, input_data, load_model_path = "train_models/age_gender_tensornets_wiki_cropface_model_20180519_123211"):
    ...
    cnn_train, cnn_net_age, cnn_net_gender, total_loss, age_loss, gender_loss, cnn_MAE_age, cnn_accuracy_gender, cnn_predictions_age, \
        cnn_predictions_gender = age_gender_model(inputs,is_training, outputs_age, outputs_gender, 80, 1, 0.0001)

    #cnn_net_age, cnn_net_gender = age_gender_model(inputs,is_training)
    ...
    temp_age_output = np.array([56])
    temp_gender_output = np.array([0])
    input_data = nets.preprocess('mobilenet100', input_data)
    feed = {inputs : input_data, outputs_age : temp_age_output, outputs_gender : temp_gender_output, is_training : False} 
    #feed = {inputs : batch_Xd, outputs_age : batch_yd_age, outputs_gender : batch_yd_gender, is_training : True}                
    
    cnn_predictions_age_test, cnn_predictions_gender_test = sess.run([cnn_accuracy_gender, cnn_predictions_gender], feed_dict=feed)
    
    #for i in range(len(cnn_predictions_age_test)):
    cnn_predictions_age_test = int(cnn_predictions_age_test) + 10
        
    return cnn_predictions_age_test, cnn_predictions_gender_test


def run_train(sess, input_dict, num_class, epochs=3, batch_size=100,print_every=10, 
    ...
    cnn_train, cnn_net_age, cnn_net_gender, total_loss, age_loss, gender_loss, cnn_MAE_age, cnn_accuracy_gender, cnn_predictions_age = \
                        age_gender_model(inputs,is_training, outputs_age, outputs_gender, num_class, batch_size, learning_rate)
